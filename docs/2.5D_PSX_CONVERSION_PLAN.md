...
- State Machine (IdleState, ExecutingTurnState) - Forward indicator system
...
/scripts/
  ...
  │   ├── player.gd
  │   └── states/ (idle, aiming, executing)
  ├── actions/ (movement_action, wait_action, action base)
  ├── grid.gd
  └── game.gd

/scenes/
  ├── game.tscn (Node2D hierarchy)
  └── main_menu.tscn
```
...
**Node Types**:
- `Node2D` → `Node3D`
- `Camera2D` → `Camera3D` (with third-person rig)
- `Label` (tiles) → `GridMap` (3D mesh instances)
- `Label` (player) → `Sprite3D` (billboard mode)

**Coordinate Mapping**:
- Grid positions: `Vector2i` (UNCHANGED)
- World positions: `Vector3` (Y = height, typically 0)
- Conversion: `(x, y)` → `(x, 0, y)` in 3D space

**Visual Pipeline**:
```
3D Scene → PSX Shaders → Camera3D → VHS Post-Process → Screen
```
...
Node3D (Game)
├─ WorldEnvironment
├─ DirectionalLight3D (ambient base light)
├─ Grid3D (Node3D)
│  └─ GridMap
├─ Player3D (CharacterBody3D)
│  ├─ CollisionShape3D
│  ├─ Model (Sprite3D) - emoji billboard
│  └─ CameraRig (Node3D)
│     └─ HorizontalPivot (Node3D)
│        └─ VerticalPivot (Node3D)
│           └─ SpringArm3D
│              └─ Camera3D
└─ UI (CanvasLayer)
   ├─ TurnCounter (Label)
...
│  └─ Mesh: PlaneMesh (1x1)
│  └─ Material: StandardMaterial3D (yellow-ish)
├─ Wall (MeshInstance3D)
│  └─ Mesh: BoxMesh (1x1x1)
│  └─ Material: StandardMaterial3D (yellow wallpaper)
└─ Ceiling (MeshInstance3D)
   └─ Mesh: PlaneMesh (1x1, rotated 180°)
   └─ Material: StandardMaterial3D (ceiling tile)
```

4. In Godot Editor: Scene → Export As → MeshLibrary
5. Save as `res://assets/grid_mesh_library.tres`

**Assign to GridMap**:
- Select GridMap node in game_3d.tscn
- Inspector → Mesh Library → Load `grid_mesh_library.tres`
- Cell Size: (1, 0.5, 1)
...
```
Player3D
└─ CameraRig (Node3D) - follows player
   └─ HorizontalPivot (Node3D) - rotation around Y-axis
      └─ VerticalPivot (Node3D) - pitch angle
...
## - 45° rotation snapping for tactical clarity
...
@export var rotation_snap: float = 45.0  # Snap to 45° increments
...
var current_rotation: int = 1  # In 45° increments (0-7)
...
func _ready() -> void:
	...
	print("[TacticalCamera] Initialized - Zoom: %.1f, Rotation: %d°" % [
		current_zoom,
...
func rotate_camera(direction: int) -> void:
	"""Rotate camera in 45° increments"""
...
- [ ] Q/E rotates camera in 45° increments
...
```
res://shaders/
├─ psx_base.gdshaderinc        # Core implementation
├─ psx_lit.gdshader            # Lit material
├─ psx_unlit.gdshader          # Unlit material
...
│  └─ Shader: res://shaders/pp_band-dither.gdshader
└─ Anchors: Full Rect (covers screen)
```
...
├─ Background
│  └─ Mode: Sky (or Color with slight yellow)
├─ Volumetric Fog
│  ├─ Enabled: true
│  ├─ Density: 0.01 (subtle)
│  ├─ Albedo: Color(0.98, 0.98, 0.95) - slight yellow tint
│  ├─ Emission Energy: 0.0
│  └─ Length: 64.0
├─ Adjustments
│  ├─ Contrast: 1.2
...
└─ Glow (optional)
   ├─ Enabled: true
   └─ Intensity: 0.3 (subtle bloom on lights)
```
...
└─ AmbientSound (AudioStreamPlayer)
   ├─ Stream: fluorescent_buzz.ogg
   ├─ Autoplay: true
   ├─ Volume: -12 dB (background level)
   └─ Bus: "Ambient"
```
...
├─ Model (Sprite3D) - NEW SETUP
│  └─ Script: player_sprite.gd
└─ CameraRig (...)
```
...
1. Create emoji images in image editor:
   - `player_idle.png` - 🚶 emoji at 128×128
   - `player_walk.png` - 🚶 emoji at 128×128
...
├─ Layout: Full Rect
├─ Material: ShaderMaterial
│  └─ Shader: vhs_post_process.gdshader
└─ Script: vhs_controller.gd
```
...
Game (Node3D)
└─ UI (CanvasLayer)
   ├─ VHSOverlay (instance of vhs_overlay.tscn) - ABOVE OTHER UI
   ├─ PostProcess (ColorRect) - PSX dither
   ├─ TurnCounter (Label)
   └─ Instructions (Label)
```
...
5. 20% chance: turn 90° left or right
...
var loaded_chunks: Dictionary = {}  # Vector2i → Array[tiles]
...
└─ Minimap (Control)
   └─ ViewportContainer
      └─ SubViewport
         └─ MinimapCamera (Camera3D, orthogonal, top-down)
```
...
- IdleState, ExecutingTurnState (AimingMoveState removed)
- Forward indicator system (camera-based movement)
- Turn boundaries preserved
- Changes: Simplified to two states, forward indicator always visible
...
**Coordinate Conversion**:
- Before: `Vector2i` → `Vector2`
- After: `Vector2i` → `Vector3` (simple mapping)
...
```
/scenes/
├─ game_3d.tscn           # Main 3D game scene
├─ vhs_overlay.tscn       # VHS post-process
└─ fluorescent_light.tscn # Light prefab

/scripts/
├─ grid_3d.gd             # 3D grid system
├─ player/
│  ├─ player_3d.gd        # 3D player controller
│  ├─ player_sprite.gd    # Billboard sprite
│  └─ tactical_camera.gd  # Camera controller
├─ procedural/
│  └─ backrooms_generator.gd  # Level generation
└─ effects/
   ├─ vhs_controller.gd   # VHS effect manager
   └─ fluorescent_light.gd # Flickering light

/shaders/
├─ psx_base.gdshaderinc   # PSX core
├─ psx_lit.gdshader       # PSX lit material
├─ psx_unlit.gdshader     # PSX unlit material
├─ pp_band-dither.gdshader # Dithering
└─ vhs_post_process.gdshader # VHS effect
...
│  ├─ psx_wall.tres
│  └─ psx_ceiling.tres
└─ grid_mesh_library.tres

/audio/
└─ fluorescent_buzz.ogg
```
